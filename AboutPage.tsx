...
const SOCIAL_LINKS = [
  { name: 'LinkedIn', url: 'https://www.linkedin.com/in/mabulhaija' },
]
...
export default function AboutPage({ lang = 'en' }: { lang?: AboutLang }) {
  ...
  useEffect(() => {
    ...
    canonical.href = `https://mta-cv.vercel.app/${t.slug}`

    const hreflangs = [
      { lang: 'en', href: 'https://mta-cv.vercel.app/about' },
      { lang: 'ar', href: 'https://mta-cv.vercel.app/ar' },
      { lang: 'x-default', href: 'https://mta-cv.vercel.app/about' },
    ]
  ...
  return (
    <div className="min-h-screen bg-background text-foreground bg-[length:24px_24px] [background-image:radial-gradient(circle,hsl(var(--dot-grid))_1px,transparent_1px)]">
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-12 md:py-20">
        ...
        {/* Manifesto */}
        <blockquote cite="https://mta-cv.vercel.app/" className="mb-10 border-l-4 border-primary pl-6 pr-4 py-3 text-xl md:text-2xl italic font-display leading-snug text-foreground/90">
          {t.manifesto}
        </blockquote>
        ...
        <div className="text-center pt-6 border-t border-border">
          ...
          >
            {lang === 'en' ? 'Read in Arabic (اقرأ بالعربية) →' : 'Read in English →'}
          </Link>
        </div>
        ...
        <footer className="mt-8 text-center">
          <p className="text-xs text-muted-foreground">
            &copy; {new Date().getFullYear()} Mohammed T. Abulhaija. {lang === 'en' ? 'All rights reserved.' : 'جميع الحقوق محفوظة.'}
          </p>
        </footer>
      </main>
    </div>
